...
**Config (`src/Config.hpp`):** `load_config()` walks `search_paths("config.json")`
— cwd first, then exe-relative with one and two `..` hops — and uses the first
it finds, so the copy at the project root serves both `cmake`-dir runs and a
double-clicked exe. The same walk finds the title's font (`title.font`), which
is why a path relative to the project root works from either. Every field is read with `value(key, default)`
...
crops to the ink so centering is on the letters, and uploads a white texture
